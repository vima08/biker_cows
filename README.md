...
Локальная браузерная игра из связанных аркадных секций: motorcycle shoot ’em up → combat-race **Sulfur Run / Road Rash** → beat ’em up **Furnace District** → финальный motorcycle shoot ’em up. Доступны три антропоморфные коровы-байкеры и локальный кооператив в shoot ’em up и beat ’em up. В Sulfur Run управляет P1; выбранная героиня P2 возвращается в Furnace District. После установки зависимостей игра использует локальные ресурсы и не обращается к внешним API. Актуальная оценка готовности и подтверждённые проблемы находятся в [DEFECTS.md](DEFECTS.md).
...
В одиночной игре: `WASD` или стрелки — движение; на байке `Z` стреляет, а в Furnace District собирает серию ударов. `X` — прыжок, `C` — спецприём, `P`/`Esc` — пауза. Поддерживаются совместимые клавиши `J/K/L`, `Space`, `Shift`, `Ctrl` и Gamepad API. В Sulfur Run `W/S` отвечают за газ/тормоз, `A/D` за руление, `Z` за боковой удар; в co-op эта секция управляется только P1.
...
- промежуточная секция кампании **Sulfur Run**: псевдо-3D combat-race на authored open-road background, отдельный atlas трафика и roadside-объектов, газ/тормоз, руление, боковые удары, масло, соперники, Road King и собственный continue-checkpoint; также доступны отдельные test scenes;
...
`hero=cassia|bruna|nova` выбирает героиню в поддерживающих её debug-сценах. Brawler-сцены `brawler-walk`, `brawler-jump` и `brawler-air-attack` фиксируют соответственно ходьбу, прыжок и воздушную атаку для визуальной/анимационной проверки. Road Rash-сцены открывают обычное движение, гарантированный ближний бой и сразу битву с Road King; каждая героиня имеет собственный задний силуэт и байк. Прямые debug URL заканчиваются самостоятельной победой; в основной кампании победа над Road King ведёт в Furnace District. `npm run test:road-rash` проверяет отдельную сцену, реальный входящий удар Кинга, восемь ответных ударов, закрытый до победы финиш, разные heroine-render signatures и continue. `npm run test:campaign` проверяет весь маршрут, включая Sulfur Run. `npm run test:debug-scenes` снимает brawler-сцены для каждой героини и проверяет `?art=vector` и runtime-переключатель арта.
